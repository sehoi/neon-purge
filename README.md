...
FPS 표시는 `업그레이드` 화면의 `FPS 표시` 버튼으로 켠다. 평균 프레임과 함께 최근 3초의 최악 프레임을 괄호 안에 보여준다 — 버벅임은 평균이 아니라 스파이크에서 드러난다.
...
업적 12개가 있다 — 무기 하나만 들고 클리어, 강화 없이 클리어, 무피격 5분 생존 같은 것들. 타이틀의 `업적` 에서 확인한다.

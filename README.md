...
가로 모드로 플레이한다(세로로 들면 회전 안내가 뜬다).

| 입력 | 동작 |
|---|---|
| 화면 왼쪽 드래그 | 이동 — 누른 자리가 조이스틱 중심 |
| 오른쪽 아래 원 | 대시 |
| 상단 중앙 `▮▮` | 일시정지 |

터치 기기로 판별되면 자동으로 조정된다.

- **화면 채우기** — 논리 폭을 화면 비율에 맞춰 늘려(1280~1750) 좌우 검은 띠를 없앤다
- **전체화면** — 판을 시작할 때 자동으로 요청한다(타이틀에 수동 버튼도 있다)
- **HUD 상단 배치** — 경험치 바·체력·레벨은 좌상단, 무기·강화 아이콘은 우상단. 하단은 손에 가린다
- **이동 속도** — 화면이 넓어진 만큼 올린다(245 → 285). 같은 속도면 더 굼떠 보인다
- **성능** — DPR 1.25 제한, 동시 적 수 60%, 격자 간격 확대, 디테일 축소 임계값 하향

...
### 배경 그리드 — 패턴으로 바꿨다가 되돌린 기록

"수평 이동 중에만 버벅인다"는 제보를 재현해보니 실제로 정지 대비 21% 무거웠다(9.5ms → 11.5ms). 화면이 가로로 넓어지면서 세로선이 20개 남짓으로 늘었고, 그 선들이 이동 중 계속 다시 그려지는 게 원인이었다.

`createPattern` + `fillRect` 한 번으로 바꾸는 게 정석처럼 보였지만 **3배 이상 느려졌다(31.9ms).** DPR 변환이 걸린 상태에서 타일을 리샘플링하는 비용이 선 스무 개를 긋는 것보다 훨씬 컸다. 되돌렸다.

실제로 통한 해법은 단순했다 — **터치 기기에서 격자 간격을 80 → 116으로 넓혔다.** 선 개수가 30% 줄면서 이동 시 추가 부하가 **4.5ms → 0.4ms**로 사라졌다. 배경이라 성겨져도 눈에 거슬리지 않는다.

교훈: 최적화는 반드시 재고 나서 채택한다. "draw call 을 줄이면 빨라진다"는 직관이 Canvas 2D 에서는 자주 틀린다.
...
- 논리 해상도는 **높이 720 고정, 폭은 화면 비율에 맞춰 1280~1900**. DPR은 데스크톱 2 / 터치 1.5로 클램프.

`config.js`의 `W`는 `const`가 아니라 **`export let`**이다. ES 모듈의 live binding 덕분에 `import { W }` 한 쪽도 갱신된 값을 본다 — 다만 **모듈 최상위에서 `W`를 읽어 상수를 계산해두면 그 값은 고정된다.** `TOUCH_UI`가 그런 경우라 `refreshTouchUI()`로 다시 계산한다. 새 코드에서 `W`를 쓸 때는 반드시 함수 안에서 읽어야 한다.
